...
namespace TcgEngine.UI
{
    ...
    public class CollectionPanel : UIPanel
    {
        ...
        public Toggle toggle_talent;
        public Toggle toggle_talent_creature;
                ...
                if (card.card.deckbuilding)
                {
                    ...
                    if (filter_team == null || filter_team == icard.team)
                    {
                        ...
                        bool type_check = (type == CardType.Character && toggle_character.isOn)
                            ...
                            || (type == CardType.Talent && toggle_talent.isOn)
                            || (type == CardType.TalentCreature && toggle_talent_creature.isOn)
                            || (!toggle_character.isOn && !toggle_spell.isOn && !toggle_artifact.isOn && !toggle_equipment.isOn && !toggle_secret.isOn && !toggle_talent.isOn && !toggle_talent_creature.isOn);
        ...
        private void RefreshDeckCards()
        {
            ...
            deck_quantity.text = CountMainDeck() + "/" + GameplayData.Get().main_deck_size
                + "  Extra: " + CountExtraDeck() + "/" + GameplayData.Get().extra_deck_size;
            bool main_ok = CountMainDeck() <= GameplayData.Get().main_deck_size;
            bool extra_ok = CountExtraDeck() <= GameplayData.Get().extra_deck_size;
            deck_quantity.color = (main_ok && extra_ok) ? Color.white : Color.red;
        ...
        public void OnClickCard(CardUI card)
        {
            ...
            if (icard != null)
            {
                int in_deck_total = CountDeckCards(icard);
                if (CanAddToDeck(icard, in_deck_total))
                {
        ...
        private bool CanAddToDeck(CardData card, int in_deck_total)
        {
            //Check type: Talent goes to extra deck, others go to main deck
            bool is_extra = (card.type == CardType.Talent);

            // Divine Mark trait (神迹): max 1 copy total in entire deck
            string divine_id = GameplayData.Get().divine_trait_id;
            if (!string.IsNullOrEmpty(divine_id))
            {
                TraitData divine = TraitData.Get(divine_id);
                if (divine != null && card.HasTrait(divine))
                    return in_deck_total < 1;
            }

            // Count how many card types already have 4 copies
            int four_of_types = CountFourOfTypes();
            bool can_have_four = four_of_types < GameplayData.Get().deck_four_of_limit;

            // Max 4 copies if we have quota, otherwise max 1
            int max_copies = can_have_four ? 4 : 1;
            if (in_deck_total >= max_copies)
                return false;

            // Check deck size limits
            if (is_extra)
                return CountExtraDeck() < GameplayData.Get().extra_deck_size;
            else
                return CountMainDeck() < GameplayData.Get().main_deck_size;
        }

        private int CountFourOfTypes()
        {
            int count = 0;
            List<string> counted = new List<string>();
            foreach (UserCardData ucard in deck_cards)
            {
                if (!counted.Contains(ucard.tid) && ucard.quantity >= 4)
                {
                    counted.Add(ucard.tid);
                    count++;
                }
            }
            return count;
        ...
        private int CountMainDeck()
        {
            int count = 0;
            foreach (UserCardData ucard in deck_cards)
            {
                CardData icard = CardData.Get(ucard.tid);
                if (icard != null && icard.type != CardType.Talent)
                    count += ucard.quantity;
            }
            return count;
        }

        private int CountExtraDeck()
        {
            int count = 0;
            foreach (UserCardData ucard in deck_cards)
            {
