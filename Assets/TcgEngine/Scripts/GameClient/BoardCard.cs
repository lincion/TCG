...
namespace TcgEngine.Client
{
    ...
    public class BoardCard : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            //Garrison display: talent card on top, garrisoned/occupied creature behind
            if (card.CardData.IsTalent() && card.talent_state != TalentState.Empty)
            {
                card_sprite.sortingOrder = 10;
                if (canvas != null) canvas.sortingOrder = 11;
            }
            else if (card.HasStatus(StatusType.Garrisoned))
            {
                card_sprite.sortingOrder = 5;
                if (canvas != null) canvas.sortingOrder = 6;
            }
            else if (card.HasStatus(StatusType.Occupied))
            {
                card_sprite.sortingOrder = 2;
                if (canvas != null) canvas.sortingOrder = 3;
            }
        ...
        private Vector3 GetTargetPos()
        {
            ...
            if (slot != null)
            {
                ...
                //Garrison offset: garrisoned creature card displays slightly offset behind talent card
                if (card.HasStatus(StatusType.Garrisoned))
                    targ_pos += new Vector3(0.4f, -0.3f, -0.01f);
