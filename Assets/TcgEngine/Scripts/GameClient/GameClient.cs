...
namespace TcgEngine.Client
{
    ...
    public class GameClient : MonoBehaviour
    {
        ...
        /// <summary>传递优先权（连锁响应时放弃发动）</summary>
        public void PassPriority()
        {
            SendAction(GameAction.PassPriority);
        }

        /// <summary>在连锁中发动效果</summary>
        public void ChainAbility(Card card, AbilityData ability)
        {
            MsgCastAbility mdata = new MsgCastAbility();
            mdata.caster_uid = card.uid;
            mdata.ability_id = ability.id;
            mdata.target_uid = "";
            SendAction(GameAction.ChainAbility, mdata);
        }

        /// <summary>设置是否跳过连锁</summary>
        public void SetSkipChain(bool skip)
        {
            MsgInt data = new MsgInt();
            data.value = skip ? 1 : 0;
            SendAction(GameAction.SkipChain, data);
        }

        /// <summary>从市场购买卡牌</summary>
        public void MarketBuy(int marketIndex)
        {
            MsgInt data = new MsgInt();
            data.value = marketIndex;
            SendAction(GameAction.MarketBuy, data);
        }

        /// <summary>从对方市场购买卡牌</summary>
        public void MarketBuyEnemy(int marketIndex)
        {
            MsgInt data = new MsgInt();
            data.value = marketIndex;
            SendAction(GameAction.MarketBuyEnemy, data);
        }

        /// <summary>送墓生物换取市场卡牌</summary>
        public void MarketSacrifice(string sacrificeCardUid, string targetCardUid)
        {
            MsgCardPair data = new MsgCardPair();
            data.card_uid = sacrificeCardUid;
            data.target_uid = targetCardUid;
            SendAction(GameAction.MarketSacrifice, data);
        }

        /// <summary>天赋卡升级</summary>
        public void TalentUpgrade(Card talentCard)
        {
            MsgCard data = new MsgCard();
            data.card_uid = talentCard.uid;
            SendAction(GameAction.TalentUpgrade, data);
        }

        /// <summary>驻守天赋卡</summary>
        public void TalentGarrison(Card talentCard, Card creature)
        {
            MsgCardPair data = new MsgCardPair();
            data.card_uid = talentCard.uid;
            data.target_uid = creature.uid;
            SendAction(GameAction.TalentGarrison, data);
        }
