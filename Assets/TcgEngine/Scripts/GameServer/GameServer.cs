...
namespace TcgEngine.Server
{
    ...
    public class GameServer
    {
        ...
        protected virtual void Init(string uid, int players, bool online)
        {
            ...
            //TCG扩展命令
            RegisterAction(GameAction.PassPriority, ReceivePassPriority);
            RegisterAction(GameAction.ChainAbility, ReceiveChainAbility);
            RegisterAction(GameAction.SkipChain, ReceiveSkipChain);
            RegisterAction(GameAction.MarketBuy, ReceiveMarketBuy);
            RegisterAction(GameAction.MarketBuyEnemy, ReceiveMarketBuyEnemy);
            RegisterAction(GameAction.MarketSacrifice, ReceiveMarketSacrifice);
            RegisterAction(GameAction.TalentUpgrade, ReceiveTalentUpgrade);
            RegisterAction(GameAction.TalentGarrison, ReceiveTalentGarrison);
        ...
        public void ReceivePassPriority(ClientData iclient, SerializedData sdata)
        {
            Player player = GetPlayer(iclient);
            if (player != null && game_data.current_priority_player == player.player_id
                && game_data.state == GameState.Play && !gameplay.IsResolving())
            {
                gameplay.PassPriority(player);
            }
        }

        public void ReceiveChainAbility(ClientData iclient, SerializedData sdata)
        {
            MsgCastAbility msg = sdata.Get<MsgCastAbility>();
            Player player = GetPlayer(iclient);
            if (player != null && msg != null
                && game_data.current_priority_player == player.player_id
                && game_data.state == GameState.Play && !gameplay.IsResolving())
            {
                Card card = player.GetCard(msg.caster_uid);
                AbilityData ability = AbilityData.Get(msg.ability_id);
                if (card != null && ability != null && card.player_id == player.player_id)
                    gameplay.PlayerChainAbility(card, ability);
            }
        }

        public void ReceiveSkipChain(ClientData iclient, SerializedData sdata)
        {
            MsgInt msg = sdata.Get<MsgInt>();
            Player player = GetPlayer(iclient);
            if (player != null && msg != null)
            {
                game_data.skip_chain = msg.value > 0;
            }
        }

        public void ReceiveMarketBuy(ClientData iclient, SerializedData sdata)
        {
            MsgInt msg = sdata.Get<MsgInt>();
            Player player = GetPlayer(iclient);
            if (player != null && msg != null && game_data.IsPlayerActionTurn(player) && !gameplay.IsResolving())
            {
                gameplay.BuyFromMarket(player, msg.value);
            }
        }

        public void ReceiveMarketBuyEnemy(ClientData iclient, SerializedData sdata)
        {
            MsgInt msg = sdata.Get<MsgInt>();
            Player player = GetPlayer(iclient);
            if (player != null && msg != null && game_data.IsPlayerActionTurn(player) && !gameplay.IsResolving())
            {
                gameplay.BuyFromEnemyMarket(player, msg.value);
            }
        }

        public void ReceiveMarketSacrifice(ClientData iclient, SerializedData sdata)
        {
            MsgCardPair msg = sdata.Get<MsgCardPair>();
            Player player = GetPlayer(iclient);
            if (player != null && msg != null && game_data.IsPlayerActionTurn(player) && !gameplay.IsResolving())
            {
                Card sacrifice = player.GetCard(msg.card_uid);
                Card target = game_data.GetCard(msg.target_uid);
                if (sacrifice != null && target != null && sacrifice.player_id == player.player_id)
                    gameplay.SacrificeForMarket(sacrifice, target);
            }
        }

        public void ReceiveTalentUpgrade(ClientData iclient, SerializedData sdata)
        {
            MsgCard msg = sdata.Get<MsgCard>();
            Player player = GetPlayer(iclient);
            if (player != null && msg != null && game_data.IsPlayerActionTurn(player) && !gameplay.IsResolving())
            {
                Card card = player.GetCard(msg.card_uid);
                if (card != null && card.player_id == player.player_id)
                    gameplay.CheckTalentUpgrade(card);
            }
        }

        public void ReceiveTalentGarrison(ClientData iclient, SerializedData sdata)
        {
            MsgCardPair msg = sdata.Get<MsgCardPair>();
            Player player = GetPlayer(iclient);
            if (player != null && msg != null && game_data.IsPlayerActionTurn(player) && !gameplay.IsResolving())
            {
                Card talentCard = player.GetCard(msg.card_uid);
                Card creature = player.GetCard(msg.target_uid);
                if (talentCard != null && creature != null
                    && talentCard.player_id == player.player_id
                    && creature.player_id == player.player_id)
                {
                    gameplay.GarrisonCard(talentCard, creature);
                }
            }
        }
