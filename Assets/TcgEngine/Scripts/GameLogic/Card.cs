...
namespace TcgEngine
{
    ...
    public class Card
    {
        ...
        //天赋卡专属属性
        public int talent_level = 0;        //天赋等级：0=Lv0(未打出), 1=Lv1, 2=Lv2, 3=Lv3
        public TalentState talent_state = TalentState.Empty; //天赋卡状态：空城/驻守/侵占
        public string garrison_card_uid = null; //驻守的生物卡UID（驻守状态时）
        public string occupy_card_uid = null;   //侵占的生物卡UID（侵占状态时）

        //行动属性
        public int action_value = 1;        //行动值（可移动格数/攻击距离）
        public bool has_moved = false;      //本回合是否已移动
        ...
        public virtual void Clear()
        {
            ...
            equipped_uid = null;
            talent_level = 0;
            talent_state = TalentState.Empty;
            garrison_card_uid = null;
            occupy_card_uid = null;
            has_moved = false;
        }
        ...
        public virtual void SetCard(CardData icard, VariantData cvariant)
        {
            ...
            hp = icard.hp;
            mana = icard.mana;
            action_value = icard.hp; //行动值 = 卡牌生命值
            SetTraits(icard);
            SetAbilities(icard);
        }
        ...
        public virtual bool CanAttack(bool skip_cost = false)
        {
            if (HasStatus(StatusType.Paralysed))
                return false;
            if (HasStatus(StatusType.CannotAttack))
                return false; //无法攻击
            if (!skip_cost && exhausted)
                return false; //no more action
            if (HasStatus(StatusType.Tapped))
                return false; //横置不可攻击
            return true;
        }
        ...
        public virtual bool CanMove(bool skip_cost = false)
        {
            if (HasStatus(StatusType.Paralysed))
                return false;
            if (HasStatus(StatusType.CannotMove))
                return false; //无法位移
            //移动后仍可攻击，但本回合已移动过则不可再移动
            if (has_moved && !skip_cost)
                return false;
            return true;
        }
        ...
        public virtual bool CanDoActivatedAbilities()
        {
            ...
            if (HasStatus(StatusType.Silenced))
                return false;
            if (HasStatus(StatusType.CannotUseEffect))
                return false; //无法使用效果
            return true;
        }
        ...
        public static void Clone(Card source, Card dest)
        {
            ...
            dest.talent_level = source.talent_level;
            dest.talent_state = source.talent_state;
            dest.garrison_card_uid = source.garrison_card_uid;
            dest.occupy_card_uid = source.occupy_card_uid;
            dest.action_value = source.action_value;
            dest.has_moved = source.has_moved;
    ...
    {
        Empty = 0,      //空城：无生物驻守/侵占
        Garrison = 1,   //驻守：己方生物驻守
        Occupied = 2,   //侵占：对方生物侵占（效果失效）
    }
