...
namespace TcgEngine
{
    ...
    public struct Slot : INetworkSerializable
    {
        ...
        public static int x_max = 7; //Number of slots in a row/zone (扩展为7列)

        public static int y_min = 1; //Dont change this, should start at 1  (0,0,0 represent invalid slot)
        public static int y_max = 5; //扩展为5行，形成5x7战场网格
        ...
        //曼哈顿距离计算（十字方向距离）
        public int ManhattanDistance(Slot slot)
        {
            return Mathf.Abs(x - slot.x) + Mathf.Abs(y - slot.y);
        }
